...
public class SanityTests {
    ...
    public void testBoard() {
        ...
        // given action.
        Board board2 = board.testMove(Board.Action.LEFT); //error!
        assertNotNull(board2);
        assertNotEquals(board, board2); //look 
        ...
        assertEquals(Board.Result.NO_PIECE, board.move(Board.Action.DOWN));//related to cp=null

        board.move(Board.Action.DOWN);
        // The row width of the bottom piece should be 4 exactly.
        assertEquals(4, board.getRowWidth(0));

        // The max row height should now be 100.
        assertEquals(1, board.getMaxHeight()); // should it be 100 or 1?
    }
}
